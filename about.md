...
The running benchmark for AI is 1) something which can run in a toaster for less than twenty dollars 2) something which is as powerful as a big model without loss in performance. 3) something which can train on-device on the chip and does not need a big cluster. Obviously, it sounds weird and stupid. That's the fun of it. Only time can tell.
<!-- 
