...
- `Flattening graphs`: The original work in semantic-parsing chose to model graphs as a `serialized form`, the basic idea was that graphs can be flattened (some magic with depth first search, and noting the parets in suffix of the sequence) and training a sequence to sequence model like a transformer. Perhaps, that does not have to be the only way. 

We have done a lot of hard work together in trying to understand this post. So we now deserve to have some fun. So, we will recall shinchan's bori bori dance from our childhood.  
