...
Today, we will learn about graph attention networks. It was a cool paper from Yoshua Bengio's lab. Their paper is [here](https://arxiv.org/pdf/1710.10903). A jupyter notebook to follow alongside this post can be found [here](https://github.com/rajatmodi62/job_study/blob/main/concept_building/mine/gat_bengio.ipynb). It has been implemented by yours truly. (Or to be honest claude implemented it, we just told it to :-)) Hiyaaa!!!

# <span style="font-size: 1.5rem; color: var(--border-header-bottom);"> Why is it important?</span>

A lot of my research is on hintons glom architecture. The key issue is how to encode part whole hierarchies in a neural net. One good way to encode hierarchies is as graphs. The key question then becomes: `how can neural nets represent graphs'?

Indeed, computer algorithms teaches us several ways: 1) there is adjacency matrix 2) there is adjacency list. The key issue is: a neural net is a memory containing massively interconnected neurons. How can this `giant mess' encode some structured informtation like a graph? Even if it manages to do so, what all sort of things could it be used for?

For the purpose of this post, we will focus on one of the cool papers from Yoshua Bengios lab called `Graph Attention Networks'. It appears that a lot of other papers have modifications to a few concepts of this paper, so it might be somewhat beneficial to spend some time on it. 
...
It looks like a spider web consisting of 34 nodes, and 78 edges. Nodes are marked as numbers from 1-34. They can have either of two colors (blue or orange). This indicates which class the node belongs to. 

Next, there is this matter of `information' in a graph. You could imagine that these nodes could be colored anything ,and any node could also connect to anything else. If there is no 'pattern' to learn, it means that even if you give me a part of the graph, you cannot predict the remaining portion. 

The above case is however different: we can see two separate clusters. If we somehow `learnt' how to split blue and orange region together, we could tell which nodes belong to which cluster. The problem then boils down to representation. So lets get started. 

We can represent our graph as an adjacency matrix. It means that each row is representing one node (let's call it A). Each column represents all the nodes that A is connected to (let's call it B). So, a value of 1 in A'th row, B'th col means that A->B. The kind of graph above is undirected, meaning that a connection goes both ways, i.e. if A->B, then B->A too. We can represent that by setting B'th row, A'th col by 1. 
...
There are a few things which stand out. The network is able to learn on just `one graph`. It only needs `two nodes` to learn, and internal message passing mechanism takes care of the rest. It is then possible to use the same network for different tasks:

- assume that a given problem, is defined by many graphs. For eg, proteins and their structure. The problem here is of `protein folding`. We can assume that we are given many proteins and their structure. Then, we can train the network at scale. Finally, given an unknown protein in the SAME problem, we can infer it's structure. We can also predict which class each node belongs to. Or, we can predict what is the angle between nodes in 3D. We can look at all the angles, and measure `torsion` or `overall mechanical stresses` on a protein. 

- The limiting factor here is that the network trained in this way will work for only ONE problem (that of protein folding). The underlying assumption is that `there are a set of weights which CAN encode the unviversal graphical structure` of all the proteins out there. As long as this remains true, the network shall learn. 


- You could also use it for recommendation systems, road networks, social graphs and whatever else you can think of. A lot of computational problems can be modelled as graphs lol. 
...
- Node i is sharing `same amount` of vibes with all the neighbors j, in the first phase of the agreement process. This might not make sense. Say you are a thin person. You go talk to a muscular guy. He will intimidate you, and you will `be afraid`. Now say, you talk to your friend, you will `be comfortable`. So, `the way you speak` depends on who you are talking to. The model above does not capture that nuance. 

- The coupling <i,j> is additive. It needs to be multiplied, but that was an issue fixed later in the variants of graphformer etc. 
...
- It requires us to know `nodes` beforehand. Say our problem is as follows: we are given an image, and we want to build graphical structure from it. This means that the input is image, and not the nodes. So, how does node formation happen `inside` the neural net. 

- `Encoding multiple graphs`: Suppose there are multiple possible graphs for the same input. How does a neural net store all of them together? There are multiple possible answers, and right now there is only one $\hat{A}$ matrix which can be used to supervise the graph. Indeed, this is a very interesting problem, and we should keep it in mind. There are some exciting ways to solve for that, so i will talk about it in another post. 

- `Flattening graphs`: The [original work](https://arxiv.org/abs/1412.7449) in semantic-parsing chose to model graphs as a `serialized form`, the basic idea was that graphs can be flattened (some magic with depth first search, and noting the node's parents in suffix of the sequence) and training a sequence to sequence model like a transformer.  

There are two conflicting arguments here: (i) we care only about breaking a sentence into parse-trees. Transformers do that quite well. so , part-whole parse-tree is solved (ii) The second argument is that we are still treating a graph as a sequence. Perhaps, that is wrong. Or maybe we are wrong :-).

Now we have finally reached the end of the post. We have done a lot of hard work together in trying to build our intuitions. So we now deserve to have some fun. So, we will recall shinchan's bori bori dance from our childhood.  
...
I love shinchan. Maybe you should too. 
